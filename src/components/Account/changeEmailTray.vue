...
function updateEmail() {
  ...
  if (confirmCurrentEmail.value !== storedEmail) {
    toast.error('Current email does not match your account.')
    return
  }
    ...
    >
      <div class="absolute top-0 right-0 text-black h-[90vh] w-9/10 shadow-xl bg-white select-none">
        <span class="bg-white flex w-full items-center border-b sticky top-0 z-10 py-2 px-4">
          <h1 class="text-2xl w-full">Change Email</h1>
        ...
        <div class="flex flex-col gap-4 p-4">
          ...
          <div>
            <Text>New Email:</Text>
            <Input v-model="newEmail" input-type="email" class="border-1 rounded-xs w-full" />
          </div>
          <button @click="updateEmail" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
            Save
          </button>
        </div>
      </div>
